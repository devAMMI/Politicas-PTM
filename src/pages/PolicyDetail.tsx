...
const PolicyDetail: React.FC<PolicyDetailProps> = ({ slug, navigate }) => {
  ...
  const docUrl = policy.document_clean_path ? buildDocCleanUrl(policy.document_clean_path) : policy.document_url;
    ...
    <main className="min-h-screen bg-[#F8F9FC]">
      ...
      {lightboxOpen && policy.cover_image_url && (
        <div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center p-4" onClick={() => setLightboxOpen(false)}>
          <button className="absolute top-4 right-4 text-white/80 hover:text-white bg-white/10 rounded-full p-2 transition-colors" onClick={() => setLightboxOpen(false)}>
            <Minimize2 size={20} />
          </button>
          <img src={policy.cover_image_url} alt={policy.title} className="max-w-full max-h-full rounded-xl shadow-2xl object-contain" onClick={e => e.stopPropagation()} />
        </div>
      )}
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <div className="px-8 md:px-10 py-5 border-b border-gray-100 flex flex-wrap gap-6">
            <div className="flex items-center gap-2 text-sm text-slate-500">
              <Calendar size={15} className="text-slate-400" /><span>{formatDateTime(policy.published_at)}</span>
            </div>
            <div className="flex items-center gap-2 text-sm text-slate-500">
              <Clock size={15} className="text-slate-400" /><span>{formatTime(policy.published_at)} hrs</span>
            </div>
            <div className="flex items-center gap-2 text-sm text-slate-500">
              <User size={15} className="text-slate-400" /><span>{policy.author_name}</span>
            </div>
          </div>
          ...
          {policy.cover_image_url && (
            <div className="px-8 md:px-10 pt-8">
              <div className="relative rounded-2xl overflow-hidden bg-slate-100 group cursor-pointer" onClick={() => setLightboxOpen(true)}>
                <img src={policy.cover_image_url} alt={policy.title} className="w-full max-h-96 object-cover transition-transform duration-300 group-hover:scale-[1.02]" />
              ...
              <div className="border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
                <div className="flex items-center justify-between px-4 py-3 bg-slate-50 border-b border-gray-200 gap-2">
                  ...
                  <div className="flex items-center gap-1.5">
                    {!isInternal && (
                      <>
                        <button onClick={() => handlePrint(false)} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-slate-600 border border-gray-200 hover:bg-white hover:text-slate-800 transition-colors">
                          <Printer size={13} /><span className="hidden sm:inline">Imprimir</span>
                        </button>
                        <a href={docUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-slate-600 border border-gray-200 hover:bg-white hover:text-slate-800 transition-colors">
                          <ExternalLink size={13} /><span className="hidden sm:inline">Abrir</span>
                        </a>
                        <button onClick={() => handleDownload(docUrl, docDisplayName)} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-slate-600 border border-gray-200 hover:bg-white hover:text-slate-800 transition-colors">
                          <Download size={13} /><span className="hidden sm:inline">Guardar</span>
                        </button>
                      </>
                    )}
                ...
                <div className="relative h-[680px]">
                  <iframe
                    ref={inlineIframeRef}
                    src={isInternal ? `${docUrl}#toolbar=0&navpanes=0&scrollbar=1&view=FitH` : `${docUrl}#toolbar=1&navpanes=1&scrollbar=1&view=FitH`}
        ...
        <div className="fixed inset-0 z-50 bg-black/95 flex flex-col">
          <div className="flex items-center justify-between px-5 py-3 bg-[#0A2647] flex-shrink-0 flex-wrap gap-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => setPdfFullscreen(false)} className="inline-flex items-center gap-1.5 bg-white/10 hover:bg-white/20 text-white text-xs font-semibold px-3 py-1.5 rounded-lg transition-all ml-2">
                <X size={14} /> Cerrar
              </button>
            </div>
          </div>
          <div className="relative flex-1" style={{ minHeight: 0 }}>
            <iframe
              ...
              src={isInternal ? `${docUrl}#toolbar=0&navpanes=0&scrollbar=1` : `${docUrl}#toolbar=1&navpanes=1&scrollbar=1`}
              className="w-full border-0"
              style={{ height: '100%', display: 'block' }}
              title={docDisplayName}
            />
            {isInternal && (
              <div className="absolute inset-0 z-10" onContextMenu={e => e.preventDefault()} style={{ background: 'transparent', userSelect: 'none', pointerEvents: 'none' }} />
            )}
          </div>
        </div>
      )}
